...
### Discovery и Node Broker {#discovery-node-broker}

Ниже показано, как динамический узел регистрируется через Node Broker, а клиент получает адреса узлов через discovery и подключается к выбранной ноде. Во время регистрации Node Broker проверяет доступность узла обратным запросом.

```mermaid
sequenceDiagram
  participant Dyn as Динамический узел
  participant NB as Node Broker
  participant Client as Клиент

  rect rgb(234, 245, 255)
    Dyn->>NB: RegisterNode (gRPC)
    NB->>Dyn: Проверка доступности (ping/check)
    Dyn-->>NB: OK
    NB-->>Dyn: Ответ: NodeId + параметры
  end

  rect rgb(240, 255, 240)
    Client->>NB: Discovery (получить адреса)
    NB-->>Client: Список endpoint'ов
    Client->>Dyn: Подключение к узлу
  end
```
